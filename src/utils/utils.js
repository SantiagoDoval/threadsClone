...
export const pointsInner = Array.from(
  ...
  const randomRadius = randomInterval(MIN_RADIUS, MAX_RADIUS);
  const randomAngle = Math.random() * Math.PI * 2;

  const x = Math.cos(randomAngle) * randomRadius;
  const y = Math.sin(randomAngle) * randomRadius;
  const z = randomInterval(-DEPTH, DEPTH);
...
export const pointsOuter = Array.from(
  ...
  const randomRadius = randomInterval(MIN_RADIUS / 2, MAX_RADIUS * 4);
  const randomAngle = Math.random() * Math.PI * 2;

  const x = Math.cos(randomAngle) * randomRadius;
  const y = Math.sin(randomAngle) * randomRadius;
  const z = randomInterval(-DEPTH * 10, DEPTH * 10);
